       ...
       <div class="sec_main-cen">
           ...
           <?php if (!empty($paginate['list'])) { ?>
               <div class="grid-movie">
                   <?php foreach ($paginate['list'] as $key => $value) {
                        $id = $value['movie_id'];
           ...
           <?php } else { ?>
               <div class="h2-text2" style="background-color: white; margin-top: 74px;">
                   <h1>ไม่พบหนังที่ค้นหา</h1>
               </div>
           <?php  } ?>
           <!-- โฆษณาล่างเนื้อหา -->
           ...
           <!-- Pagination -->
           <?php //echo "<pre>";print_r($paginate);die; 
            ?>
           <?php if ($paginate['total_page'] >= 2) { ?>
               <div class="box">
