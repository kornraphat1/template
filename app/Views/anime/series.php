	...
	<div class="content-main-av">
		...
		<div class="box">
			<div class="filmicerik">
				...
				<div id="sound_sub" class="sound_container">
					<?php
					foreach ($video_data['ep_data'] as $key => $value) {
						$url_name =  urldecode(trim(str_replace(")", "", (str_replace("(", "", (str_replace(" ", "-", $video_data['movie_thname'])))))));
						$url_epname =  urldecode(trim(str_replace(")", "", (str_replace("(", "", (str_replace(" ", "-", $value['NameEp'])))))));
					?>
						<p class="series-list"><a href="<?php echo base_url() . '/anime/video/' . $video_data['movie_id'] . '/' . $url_name . '/' . $key . '/' . $url_epname ?>"> <?php echo $video_data['movie_thname'] . ' - ' . $value['NameEp'] ?> </a></p>
					<?php
					}
					?>
				</div>
		...
		<div class="box">
			...
			<?php foreach ($vdorandom as  $value) {
				...
				$urlvideo = base_url('/anime/video/' . $id . '/' . $title_encode);
			...
			<ul>
				<?php foreach ($category_id as $value) { ?>
					<li class="cat-item cat-left">
						<h3 class="cate-name">
							<a href="<?php echo base_url('/anime/category/' . $value['category_id'] . '/' . urlencode(str_replace(' ', '-', $value['category_name']))); ?>" title=<?= $value['category_name'] ?>>
								<?= $value['category_name'] ?>
							</a>
						</h3>
					</li>
