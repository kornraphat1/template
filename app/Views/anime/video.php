<div id="content">
	...
	<div class="content-main-av">
		<div class="box">
			...
			<div class="movie-header">
				<div class="movie-thumbnail">
					<img src="<?php echo $movie_picture; ?>" alt="<?php echo $video_data['movie_thname']; ?>" title="<?php echo $video_data['movie_thname']; ?>">
				</div>
			...
			<div class="filmicerik">
				...
				<div style="display: block; padding: 15px;" class="movie_player">
					<div class="video-info">
						...
						<div class="row">
							<div class="col-lg-3">
								<button class="pull-right btn report-movie" onclick="goReport('<?= $video_data['movie_id'] ?>','<?= $video_data['branch_id'] ?>','<?= $video_data['movie_thname'] ?>','<?= $video_data['ep_data'][$index]['NameEp'] ?>')">
									<a style="color: aliceblue;" >แจ้งหนังเสีย</a>
								</button>
							</div>
						</div>
						<!-- Your share button code -->
						<!-- ********************* SOURCS MOVIES  ****************************  -->
						<div class="embed-responsive embed-responsive-16by9 video-embed-box" style="height: 520px; margin-bottom: 10px;">
							<iframe style="width:100%; height:100%; z-index:500;" id="player" class="player" src="<?= base_url('anime/player/' . $video_data['movie_id'] . '/' . $feildplay . '/' . $index) ?>" scrolling="no" frameborder="0" allowfullscreen="yes"></iframe>
				...
				if ($video_data['movie_type'] == 'se') {
					...
					// echo '<pre>', print_r($video_data['ep_data'], true), '</pre>';
					// die;
				?>
					<div class="movie-series-content ">
						<div class="row">
							<div class="col-md-12">
								...
								if ($index > 0) {
									$url_name =  urldecode(trim(str_replace($str_replace_s, $str_replace_n, $video_data['movie_thname'])));
									$key = $index - 1;
									$url_epname =  urldecode(trim(str_replace($str_replace_s, $str_replace_n, $video_data['ep_data'][$key]['NameEp'])));
									$disabled = '';
								} else {
									$url_name =  urldecode(trim(str_replace($str_replace_s, $str_replace_n, $video_data['movie_thname'])));
									$key = $index;
								...
								<a href="<?php echo base_url() . '/anime/video/' . $video_data['movie_id'] . '/' . $url_name . '/' . $key . '/' . $url_epname ?>"><button <?= $disabled ?> style=" float: left;">ตอนก่อนหน้า</button></a>


								<select onchange="click_ep(this)">

									<?php
									foreach ($video_data['ep_data'] as $keys => $value) {
										$url_epname =  urldecode(trim(str_replace($str_replace_s, $str_replace_n, $value['NameEp'])));
										$select = "";
										if ($keys == $video_data['ep_data'][$index]) {
											$select = 'selected';
										}
										// $href="<?php echo base_url() . 'anime/video/' . $video_data['movie_id'] . '/' . $url_name . '/' . $key . '/' . $url_epname 
									?>
										<option value="<?php echo $url_name . '/' . $keys . '/' . $url_epname ?>" <?= $select; ?>><?php echo $video_data['movie_thname'] . ' - ' . $value['NameEp'] ?> </option>
									<?php } ?>
								</select>


								<?php
								if (isset($video_data['ep_data'][$index + 1])) {
									$url_name =  urldecode(trim(str_replace($str_replace_s, $str_replace_n, $video_data['movie_thname'])));
									$key = $index + 1;
									$url_epname =  urldecode(trim(str_replace($str_replace_s, $str_replace_n, $video_data['ep_data'][$key]['NameEp'])));
									$disabled = '';
								...
								?>
								<a href="<?php echo base_url() . '/anime/video/' . $video_data['movie_id'] . '/' . $url_name . '/' . $key . '/' . $url_epname ?>"><button style=" float: right; " <?= $disabled ?>>ตอนถัดไป</button></a>
							</div>
						</div>
					</div>
				<?php
		...
		<div class="box" style="position: relative;">
			...
			<div class="fb-comments" data-href="<?= base_url(uri_string()) ?>" data-colorscheme="dark" data-width="100%" data-numposts="5"></div>
			<div id="fb-root"></div>
			<script>
				(function(d, s, id) {
					var js, fjs = d.getElementsByTagName(s)[0];
		...
		<div class="box">
			...
			<?php foreach ($vdorandom as  $value) {
				...
				$urlvideo = base_url('/anime/video/' . $id . '/' . $url_name);
	...
	<script>
		function click_ep(selectObject) {
			...
			// alert(value)
			window.location.href = "<?= base_url() ?>/anime/video/<?= $video_data['movie_id'] ?>/" + value;
		}
	</script>