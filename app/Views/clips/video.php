	...
	<div class="content-main-av">
			...
			<div class="filmicerik">
				...
				<div style="display: block; padding: 15px;" class="movie_player">
					<div class="video-info">
						...
						<div class="row">
							<div class="col-lg-3">
								<button class="pull-right btn report-movie" onclick="goReport('<?= $video_data['movie_id'] ?>','<?= $video_data['branch_id'] ?>','<?= $video_data['movie_thname'] ?>')">
						...
						<div class="embed-responsive embed-responsive-16by9 video-embed-box" style="height: 520px; margin-bottom: 10px;">
							<!-- <iframe style="width:100%; height:100%; z-index:500;" id="player" class="player" src="<?= base_url('/clips/player/' . $video_data['movie_id'] . '/' . $feildplay . '/' . $index) ?>" scrolling="no" frameborder="0" allowfullscreen="yes"></iframe> -->
							<?php
							// echo '<pre>'.print_r($videodata ,true).'</pre>';die;


							echo $video_data['movie_ensub1']; ?>
						</div>
						<!-- ********************* SOURCS MOVIES  ****************************  -->
					</div>
					<!-- /Movie source -->
		...
		<div class="box">
			...
			<?php foreach ($vdorandom as  $value) {
				...
				$urlvideo = base_url('/clips/video/' . $id . '/' . $url_name);
			...
			<ul>
				<?php foreach ($category_id as $value) { ?>
					<li class="cat-item cat-left">
						<h3 class="cate-name">
							<a href="<?php echo base_url('/clips/category/' . $value['category_id'] . '/' . urlencode(str_replace(' ', '-', $value['category_name']))); ?>" title=<?php echo $value['category_name']; ?> alt="<?php echo $value['category_name']; ?>">
